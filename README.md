...
```text
CyberSec-Toolkit/
├── core/               # Ядро: база данных, конфигурация, утилиты, логгер
├── modules/            # Модули (recon, dns, web, network, wireless, pivot...)
├── plugins/            # Плагины с hot-reload (шаблон: _template.py)
├── tests/              # Тесты
├── tui/                # Textual TUI (меню, статусбар, hotkeys)
├── utils/              # Вспомогательные утилиты (сеть, кодирование, файлы)
├── wordlists/          # Словари для брута и фаззинга
...
├── cyber_toolkit.db    # SQLite-база (сканы, findings, vault)
├── fix_plugin_actions.py # Утилита для плагинов
├── main.py             # Точка входа (CLI / TUI)
├── requirements.txt    # Python-зависимости
├── schedule.yaml       # Конфиг планировщика задач
├── start.bat           # Запуск на Windows (установка + CLI)
